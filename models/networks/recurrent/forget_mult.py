...
class GPUForgetMult(torch.autograd.Function):
    ...
    def compile(self):
        if self.ptx is None:
            # https://github.com/salesforce/pytorch-qrnn/pull/25/commits/92f2d5272e30fa4d01880f001f7e55c5d8e6aab8
            # removed kernel.encode() and 'rec...'.encode() based on above
...
if __name__ == '__main__':
    ...
    a = Variable(torch.rand(seq, batch, hidden).cuda(), requires_grad=True).cuda()
    forget = Variable(torch.rand(seq, batch, hidden).cuda(), requires_grad=True).cuda()
    last_h = Variable(torch.rand(batch, hidden).cuda(), requires_grad=True).cuda()
    ...
    inputs = [forget.double(), a.double(), last_h.double()]
    test = gradcheck(ForgetMult(), inputs, eps=1e-4, atol=1e-2, raise_exception=True)
    print(test)