...
def entropy(x, k=3, base=2):
    ...
    const = torch.digamma(n_elements) - torch.digamma(k) + n_features * log(2)
...
def mi(x, y, z=None, k=3, base=2):
    ...
    if z is None:
        a, b, c, d = avgdigamma(x, dvec), avgdigamma(y, dvec), torch.digamma(k), torch.digamma(len(x))
    else:
        xz = np.c_[x, z]
        yz = np.c_[y, z]
        a, b, c, d = avgdigamma(xz, dvec), avgdigamma(yz, dvec), avgdigamma(z, dvec), torch.digamma(k)
...
def avgdigamma(points, dvec):
    ...
    for point, dist in zip(points, dvec):
        ...
        avg += torch.digamma(num_points) / n_elements
    return avg
