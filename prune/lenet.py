# https://github.com/larry0123du/PyTorch-Deep-Compression

from configs.models.lenet import *
from merge.merger import init_layer_fuser


class Compressor(object):

    def __init__(self, model, cuda=False, perc=0.5):
        self.model = model
        self.num_layers = 0
    ...
    def prune(self):
        ...
        for m in self.model.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
                ...
                if self.cuda: weight_mask = weight_mask.cuda()
                self.weight_masks[index] = weight_mask

                bias_mask = torch.ones(m.bias.data.size())
                if self.cuda: bias_mask = bias_mask.cuda()
    ...
    def fuse(self):
        ''' :return: percentage fused in the network '''
        self.model, self.layers_fused = init_layer_fuser(self.model, layers=['linear', 'conv'],
                                      perc=self.perc, dist='cosine', fuse_method='mix')
        return self.perc
    ...
    def set_fuse_grad(self):
        index = 0
        for name, m in self.model.named_modules():
            if name in self.layers_fused:
                m.weight.grad.data *= torch.zeros(m.weight.size())
                m.bias.grad.data *= torch.zeros(m.bias.size())
                index += 1
